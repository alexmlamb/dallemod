...
class EncoderBlock(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        #c = make_conv(self.n_in,  self.n_hid, 3)
        #c2 = Conv2d(1,1,1)
        #print('c params', c._parameters)
        #print('c2 params', c2._parameters)

        self.res_path = nn.Sequential(OrderedDict([
                ('relu_1', nn.ReLU()),
                ('conv_1', sn(make_conv(self.n_in,  self.n_hid, 3),name='w')),
                ('relu_2', nn.ReLU()),
...
class Encoder(nn.Module):
    group_count:     int = 4
    n_hid:           int = attr.ib(default=64,  validator=lambda i, a, x: x >= 64)
    n_blk_per_group: int = attr.ib(default=2,    validator=lambda i, a, x: x >= 1)
    input_channels:  int = attr.ib(default=1,    validator=lambda i, a, x: x >= 1)
    vocab_size:      int = attr.ib(default=32, validator=lambda i, a, x: x >= 32)
